...
This section will give an overview of several background isolation algorithms.\\
...
The Mixture of Gaussians method (MOG) has $O(m)$ complexity, where $m$ is the number of Gaussian distributions used, typically 3 to 5. MOG has a high accuracy \cite{Piccardi2004}.\\


\subsubsection{Running Gaussian average}
The fastest amongst the methods reviewed by Piccardi was the Running Gaussian Average (RAG), having a time complexity of $O(1)$. RAG has a low/medium accuracy \cite{Piccardi2004}.\\

\subsubsection{Temporal Median Filter}
Temporal Median Filter (TMF) has a similar classification cost to RAG, but updating the  model is approximated as linear in the number of samples, $n$, therefore the corresponding complexity is consider $O(n)$. TMF also has a similar accuracy to RAG, low/medium \cite{Piccardi2004}.\\

\subsubsection{Kernel Density Estimation}
The Kernel Density Estimation (KDE) model computes its value in the Gaussian kernels centred on the past $n$ frames, thus the complexity is $O(n)$, where $n$ is typically as high as 100. However, efficient implementation through the Fast Gauss transform can limit the actual execution time \cite{Elgammal2003}. KDE has a high accuracy \cite{Piccardi2004}.\\

\subsubsection{Sequential Kernel Density approximation}
The Sequential Kernel Density approximation (SKDA) method has $O(m)$ complexity, where $m$ is the number of modes of
...
The complexity for the Cooccurrence of Image Variations (CIV) method has been
...
The Eigenbackground method (EB) has an estimated complexity per pixel of $O(m)$, where $m$ is the number of the best eigenvectors. EB has a medium accuracy \cite{Piccardi2004}.\\
