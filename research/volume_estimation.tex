...
It should be noted that in this method the LIDAR emitter was above the target \cite{Maltamo2006}, rather than in front of them as is likely for the project case. This method then translate to using the top and the bottom most point of the cloud to calculate height. However it may be able to be extended to calculate metrics such as depth and breath, from which a minimum bounding box could be determined.\\
...
For the purposes of PARSE, this box would be three dimensional and the volume of the minimum bounding box could be calculated as in Figure \ref{fig:calculating_the_volume_of_the_minimum_bounding_box}.\\
